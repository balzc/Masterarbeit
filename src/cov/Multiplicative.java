...
public class Multiplicative extends CovarianceFunction{
	public DoubleMatrix parameters;
	public int numParams;
	public CovarianceFunction cov1;
	public CovarianceFunction cov2;
	
	public Multiplicative(CovarianceFunction c1, CovarianceFunction c2){
		cov1 = c1;
		cov2 = c2;
	...
	public DoubleMatrix computeSingleValue(DoubleMatrix parameters, DoubleMatrix X){
		...
		return cov1.computeSingleValue(p1,X).mul(cov2.computeSingleValue(p2, X));
	}
	...
	public DoubleMatrix computeDerivatives(DoubleMatrix parameters, DoubleMatrix X, int index) {
		...
		if(index < cov1.getNumParams()){
			return cov1.computeDerivatives(p1, X, index).mul(cov2.computeSingleValue(p2, X));
		} else{
			return cov2.computeDerivatives(p2, X, index-cov1.getNumParams()).mul(cov1.computeSingleValue(p1, X));
		}
